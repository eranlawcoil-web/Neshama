...
const Hero: React.FC<HeroProps> = ({ profile, isAdmin, onUpdateProfile, onPlayStory, isCandleLit, setIsCandleLit, onShowQR }) => {
  ...
  return (
    <div className="relative w-full bg-white text-stone-900 overflow-hidden pb-12 transition-all border-b border-stone-200">
      {/* Background with Parallax effect - Lightened for readability */}
      <div className="absolute inset-0 z-0 h-[600px]">
        <img 
          src={profile.heroImage} 
          alt={profile.fullName} 
          className="w-full h-full object-cover opacity-20 blur-sm scale-105 grayscale-[20%]"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-white via-white/70 to-white/40"></div>
      </div>
      
      {/* Realistic Candle Animation Styles - Adjusted for Light Background */}
      <style>{`
        .candle-container {
            width: 80px;
            height: 100px;
        ...
        .flame {
            ...
            transform: translateX(-50%);
            box-shadow: 
                0 0 10px 2px rgba(255, 165, 0, 0.5), 
                0 0 20px 5px rgba(255, 215, 0, 0.3), 
                0 -10px 20px 5px rgba(255, 69, 0, 0.2); 
      ...
      <div className="relative z-10 container mx-auto px-4 pt-24 flex flex-col items-center text-center">
        ...
        <div className="relative group mb-8">
          <div className="w-48 h-48 md:w-64 md:h-64 rounded-full border-[6px] border-white shadow-2xl overflow-hidden p-1 bg-white ring-1 ring-stone-200 relative">
            <img 
              src={profile.heroImage} 
              alt={profile.fullName} 
          ...
          {isAdmin && !isEditingMode && (
            <label className="absolute bottom-4 right-4 cursor-pointer bg-white text-stone-900 p-3 rounded-full shadow-xl border border-stone-100 hover:bg-amber-500 hover:text-white transition-all transform hover:scale-110 z-20">
              <Camera size={22} />
        ...
        {isEditingMode ? (
          <div className="w-full max-w-2xl bg-white/90 backdrop-blur-md p-8 rounded-3xl border border-stone-200 shadow-2xl animate-in fade-in slide-in-from-bottom-4 text-right">
             <h3 className="text-amber-600 font-bold mb-6 text-xl flex items-center justify-center gap-2 border-b border-stone-100 pb-4">
               <Edit3 size={20}/> עריכת פרופיל ראשי
             </h3>
             
             {/* Edit Main Image Button (Also in Edit Mode) */}
             <div className="flex justify-center mb-6">
                 <label className="cursor-pointer bg-stone-100 hover:bg-stone-200 text-stone-700 px-4 py-2 rounded-lg border border-stone-300 flex items-center gap-2 transition-colors">
                    <Camera size={16} />
             ...
             <div className="space-y-4 mb-6">
                <div>
                  <label className="text-xs text-stone-500 block mb-1">שם המונצח</label>
                  <input 
                      value={editForm.fullName} 
                      onChange={e => setEditForm({...editForm, fullName: e.target.value})}
                      className="w-full bg-stone-50 border border-stone-300 rounded-lg p-3 text-lg text-stone-900 focus:border-amber-500 outline-none transition-colors"
                  />
                </div>

                <div className="flex gap-4">
                  <div className="flex-1">
                      <label className="text-xs text-stone-500 block mb-1">תאריך לידה</label>
                      <input 
                          type="date"
                          value={editForm.birthDate} 
                          onChange={e => handleDateChange('birthDate', e.target.value)}
                          className="w-full bg-stone-50 border border-stone-300 rounded-lg p-3 text-stone-900 focus:border-amber-500 outline-none transition-colors"
                      />
                  </div>
                  <div className="flex-1">
                      <label className="text-xs text-stone-500 block mb-1">תאריך פטירה</label>
                      <input 
                          type="date"
                ...
                <div>
                  <label className="text-xs text-stone-500 block mb-1">משפט מפתח (כותרת משנה)</label>
                  <input 
             ...
             <div className="mb-8">
                <div className="flex justify-between items-end mb-2">
                    <label className="text-xs text-stone-500">ביוגרפיה / הספד</label>
                    <div className="flex gap-2 items-center bg-stone-100 rounded-lg p-1 pr-2 border border-stone-200">
                         <input 
                            placeholder="נושאים לכתיבה..." 
                            value={keywords}
                            onChange={e => setKeywords(e.target.value)}
                            className="bg-transparent border-none text-xs w-32 outline-none text-stone-800 placeholder-stone-400"
                         />
                ...
                <textarea 
                   ...
                   className="w-full bg-stone-50 border border-stone-300 rounded-lg p-4 text-stone-800 h-40 focus:border-amber-500 outline-none resize-none leading-relaxed"
                />
             </div>

             {/* Family Members Editor Section */}
             <div className="mb-8 bg-stone-50 p-5 rounded-xl border border-stone-200">
                <div className="flex items-center gap-2 mb-4 border-b border-stone-200 pb-2">
                   <Users size={18} className="text-amber-600"/>
                   <label className="text-sm font-bold text-stone-700">קרובים ואתרי הנצחה</label>
                </div>
                
                <div className="space-y-3 mb-6 max-h-48 overflow-y-auto custom-scrollbar">
                  {familyMembers.map(m => (
                    <div key={m.id} className="flex items-center justify-between bg-white p-3 rounded-lg border border-stone-200 shadow-sm">
                       <div className="flex items-center gap-3">
                         <img src={m.imageUrl || 'https://via.placeholder.com/40'} alt={m.name} className="w-10 h-10 rounded-full object-cover ring-1 ring-stone-200" />
                         <div className="text-right">
                            <div className="text-sm font-bold text-stone-800">{m.name} <span className="text-amber-500 font-normal text-xs px-1">|</span> <span className="text-stone-500 font-normal text-xs">{m.relation}</span></div>
                            <div className="text-xs text-stone-500 truncate max-w-[150px]">{m.shortDescription}</div>
                         </div>
                       </div>
                       <button onClick={() => removeFamilyMember(m.id)} className="bg-red-50 hover:bg-red-100 text-red-500 p-2 rounded-lg transition-colors border border-red-100">
                          <Trash2 size={16} />
                       </button>
                    </div>
                  ))}
                  {familyMembers.length === 0 && <div className="text-stone-400 text-sm italic text-center py-2">לא הוספו עדיין קרובים.</div>}
                </div>

                <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-inner">
                    <p className="text-xs text-stone-500 mb-3 font-bold flex items-center gap-1"><Plus size={12}/> הוספת פרופיל קשור חדש:</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                       <input 
                            ...
                            placeholder="שם מלא *"
                            className="bg-stone-50 border border-stone-300 rounded px-3 py-2 text-sm text-stone-900 focus:border-amber-500 outline-none"
                        />
                        <input 
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                         <div className="flex flex-col">
                             ...
                             <input 
                                ...
                                className="bg-stone-50 border border-stone-300 rounded px-2 py-1.5 text-xs text-stone-900 focus:border-amber-500 outline-none"
                             />
                         </div>
                         <div className="flex flex-col">
                    ...
                    <textarea 
                        ...
                        rows={2}
                        className="w-full bg-stone-50 border border-stone-300 rounded px-3 py-2 text-sm text-stone-900 mb-3 focus:border-amber-500 outline-none resize-none"
                    />
                    ...
                    <input 
                        ...
                        className="w-full bg-stone-50 border border-stone-300 rounded px-3 py-2 text-sm text-stone-900 mb-3 dir-ltr text-right focus:border-amber-500 outline-none"
                    />
                    
                    <div className="flex items-center gap-3 pt-2 border-t border-stone-100 mt-2">
                        <label className={`cursor-pointer px-3 py-2 rounded text-xs flex items-center gap-2 transition-colors border ${newFamilyMember.imageUrl ? 'bg-green-50 border-green-200 text-green-700' : 'bg-stone-100 border-stone-200 text-stone-600 hover:bg-stone-200'}`}>
                            <Camera size={14} /> 
                            {newFamilyMember.imageUrl ? 'תמונה נבחרה' : 'בחר תמונה'}
                            <input type="file" className="hidden" onChange={handleFamilyMemberImage} />
                        </label>
                        <div className="flex-1"></div>
                        <button onClick={addFamilyMember} className="bg-stone-200 hover:bg-stone-300 text-stone-900 px-4 py-2 rounded font-bold text-sm flex items-center gap-1 shadow-sm transition-all transform hover:scale-105">
             ...
             <div className="flex gap-3 pt-4 border-t border-stone-100">
               <button onClick={() => setIsEditingMode(false)} className="flex-1 py-3 bg-stone-100 hover:bg-stone-200 text-stone-600 rounded-xl font-bold transition-colors">ביטול עריכה</button>
               <button onClick={saveMainDetails} className="flex-1 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-bold shadow-lg shadow-amber-200 transition-all hover:scale-[1.02] flex items-center justify-center gap-2">
          ...
          <>
            <div className="flex items-center justify-center gap-4 mb-4 group relative">
              <h1 className="text-5xl md:text-8xl font-serif-hebrew font-bold tracking-wide text-stone-900 drop-shadow-sm leading-tight">
                {profile.fullName}
              </h1>
              ...
              <div className="flex gap-2 absolute left-0 md:-left-24 top-1/2 -translate-y-1/2">
                {isAdmin && (
                    <button
                      ...
                      className="bg-white hover:bg-stone-100 text-amber-600 hover:text-amber-700 p-3 rounded-full shadow-lg border border-stone-200 opacity-0 group-hover:opacity-100 transition-opacity"
                      title="ערוך פרטים"
                    >
                      <Edit3 size={24} />
                    </button>
                ...
                <button
                  onClick={handleShare}
                  className="bg-white hover:bg-stone-100 text-stone-500 hover:text-amber-600 p-3 rounded-full shadow-lg border border-stone-200 opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            {profile.shortDescription && (
                <p className="text-lg md:text-2xl text-stone-700 font-serif-hebrew max-w-2xl mx-auto mb-10 leading-relaxed">
                    {profile.shortDescription}
                </p>
            )}
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-16 relative z-20">
                <button 
                    ...
                    className="group flex items-center gap-2 bg-white/60 backdrop-blur-md hover:bg-white border border-stone-200 px-6 py-3 rounded-full transition-all hover:scale-105 shadow-sm"
                >
                    <Info size={18} className="text-amber-600 group-hover:rotate-12 transition-transform" />
                    <span className="font-medium text-stone-800">פרטים ומיקום</span>
                </button>
                ...
                {profile.playlistUrl && (
                    <a 
                        ...
                        className="group flex items-center gap-2 bg-white/60 backdrop-blur-md hover:bg-white border border-stone-200 px-6 py-3 rounded-full transition-all hover:scale-105 shadow-sm"
                    >
                        <Music size={18} className="text-green-600 group-hover:scale-110 transition-transform" />
                        <span className="font-medium text-stone-800">הפלייליסט שלי</span>
                    </a>
                )}

                <button 
                    onClick={onPlayStory}
                    className="flex items-center gap-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white px-8 py-3 rounded-full transition-all shadow-lg hover:shadow-amber-200 hover:scale-105 active:scale-95"
                >
            ...
            <div className="max-w-4xl mx-auto relative group">
              <div className="relative p-8 md:p-12 bg-white rounded-3xl border border-stone-100 shadow-xl mb-12">
                 <span className="text-7xl text-amber-500/20 absolute -top-8 -right-6 font-serif select-none">❝</span>
                 <p className="text-lg md:text-xl font-heebo font-light leading-relaxed text-stone-800 whitespace-pre-line text-right">
                    {profile.bio}
                 </p>
                 <span className="text-7xl text-amber-500/20 absolute -bottom-10 -left-6 font-serif select-none">❞</span>
              </div>
            </div>
          </>
        )}
