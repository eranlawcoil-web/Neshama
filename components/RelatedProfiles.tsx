...
const RelatedProfiles: React.FC<RelatedProfilesProps> = ({ relatedPeople }) => {
  ...
  return (
    <div className="w-full bg-stone-100 text-stone-800 py-24 px-4 border-t border-stone-200 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-16">
          <div className="inline-block p-3 rounded-full bg-white mb-4 border border-stone-200 shadow-sm">
            <Heart className="w-6 h-6 text-amber-600" fill="currentColor" fillOpacity={0.2} />
          </div>
          <h2 className="text-3xl md:text-5xl font-serif-hebrew text-stone-800 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {relatedPeople.map((person) => (
            <div 
              key={person.id} 
              className="group relative bg-white rounded-2xl overflow-hidden border border-stone-200 shadow-lg hover:shadow-xl transition-all duration-500 hover:-translate-y-1 h-96"
            >
              {/* Image & Overlay */}
              ...
              <div className="absolute inset-0 z-10 p-6 flex flex-col justify-end text-white">
                  ...
                  <div className="transform transition-transform duration-500 translate-y-4 group-hover:translate-y-0">
                      <div className="flex justify-between items-end mb-2">
                           <span className="inline-block text-amber-300 font-bold text-xs tracking-wider uppercase bg-black/50 px-3 py-1 rounded-full backdrop-blur-md border border-white/20">
                              {person.relation}
                           </span>
                           <span className="text-xs font-mono text-stone-300 opacity-90 dir-ltr shadow-black drop-shadow-md">
                      ...
                      {person.shortDescription && (
                        <p className="text-stone-200 text-sm leading-relaxed mb-4 line-clamp-2 font-medium drop-shadow-sm">
                            {person.shortDescription}
                        </p>
                      )}
                      ...
                      <div className="h-0 group-hover:h-12 opacity-0 group-hover:opacity-100 transition-all duration-500 overflow-hidden">
                          {person.memorialUrl && person.memorialUrl !== '#' ? (
                            <a 
                                ...
                                className="flex items-center justify-center gap-2 w-full py-3 bg-white text-stone-900 hover:bg-amber-50 rounded-xl font-bold text-sm shadow-lg transition-colors"
                            >
                                <span>ביקור באתר ההנצחה</span>
                                <ExternalLink size={14} />
                            </a>
                           ) : (
                            <div className="flex items-center justify-center gap-2 w-full py-3 bg-black/40 text-stone-300 rounded-xl font-medium text-sm border border-white/20 cursor-default backdrop-blur-sm">
